...
| 4 | HUMANS.md | "≡ (congruence) — same structure and depth, possibly built by different paths." | Ch20 and AGENTS.md corrected ≡ to depth-free: "Same outcome regardless of derivation depth or path." HUMANS.md still carries the old depth-dependent definition, contradicting the agent-facing spec and the code (`relation_congruent` in relations.py is depth-free). | Update HUMANS.md ≡ definition to "same outcome regardless of derivation depth or path." | open | 2026-04-10 |
| 5 | HUMANS.md | Relations section says "These go from strictest to loosest" and lists =, ≡, ~ in that order. | Factually correct as presentation ordering, but silently omits the foundational claim in AGENTS.md: "Dependence runs downward from ~: similarity is foundational, equality is derived." The human-facing doc presents = first (implying primacy) while the agent-facing doc establishes ~ as primary. A reader of HUMANS.md alone would not know = is derived. | Add one sentence: dependence runs from ~, not =. Similarity is foundational; equality is earned by closure. | open | 2026-04-10 |
