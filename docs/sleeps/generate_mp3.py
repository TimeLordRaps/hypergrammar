...
# Words per synthesis chunk. VoxCPM2 handles long inputs but chunking keeps
# memory pressure low and allows progress visibility.
CHUNK_WORDS = 400
...
def main() -> None:
    ...
    chunks = split_into_chunks(raw_text, CHUNK_WORDS)
    print(f"  Split into {len(chunks)} synthesis chunks")
    ...
    for i, chunk in enumerate(chunks, 1):
        word_count = len(chunk.split())
        print(f"  Synthesizing chunk {i}/{len(chunks)} ({word_count} words)...")
        # Voice design prefix on every chunk — each model.generate() call is
        # independent; the style descriptor ensures consistent timbre/pace throughout.
        text_input = VOICE_STYLE + chunk
        wav = model.generate(
            text=text_input,
            cfg_value=2.0,
            inference_timesteps=10,
    ...
    print(f"  Total duration: {duration_sec / 60:.1f} min")

    print(f"Saving WAV → {WAV_FILE.name}...")
    ...
    try:
        ...
        print(f"Converting WAV → MP3 ({MP3_FILE.name})...")
        seg = AudioSegment.from_wav(str(WAV_FILE))
        seg.export(str(MP3_FILE), format="mp3", bitrate="192k")
        print("MP3 saved.")
        os.remove(WAV_FILE)
        ...
        print("pydub not installed — WAV artifact kept, MP3 conversion skipped.")
        print(f"To finish manually: ffmpeg -i {WAV_FILE} -b:a 192k {MP3_FILE}")
    except Exception as exc:  # noqa: BLE001
        print(f"MP3 conversion failed ({exc}) — WAV artifact kept.")
        print(f"To finish manually: ffmpeg -i {WAV_FILE} -b:a 192k {MP3_FILE}")

    print(f"\nDone. {duration_sec / 60:.1f} min of audio at {sample_rate} Hz.")
